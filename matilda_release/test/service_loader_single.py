...
def load_service_data(plugin_name=None, delete_unused_services=False, delete_unused_actions=False, delete_unused_service_fields=False, delete_unused_output_fields=False):
    if plugin_name is None:
        raise Exception('Plugin Name is mandatory for single service loader')
    ...
    overall_common_output_fields = os.path.join(path, 'common_plugin_output.json')
    ...
    info = os.path.join(plugin_path, 'plugin.info')
    details =os.path.join(plugin_path, 'plugin.json')
    services = []
    ...
    with open(details, "rb") as fin:
        ...
        for ac in ac_ip:
            ...
            for output_field_details in output_field_list:
                ...
                output_field['output_field_id'] = service['service_id'] + '_' + action_details.get(
                    'action_id') + '_' + output_field_details.get('output_field_id')
            ...
            for sf in sf_ip:
                sf_op = {}
                sf_op['service_field_id'] = service['service_id'] + '_' + action_details.get(
                    'action_id') + '_' + sf.get('service_field_id')
    ...
    if services is not None and len(services) > 0:
        service_handler.create_service_stack(args=services, delete_unused_services=delete_unused_services,
                                             delete_unused_actions=delete_unused_actions,
                                             delete_unused_service_fields=delete_unused_service_fields,
                                             delete_unused_output_fields=delete_unused_output_fields)
        return services


load_service_data(plugin_name='agile_central', delete_unused_actions=True, delete_unused_service_fields=True,
                  delete_unused_output_fields=True)
